...
const BrokerTenants = () => {
    ...
    // In a real app, this would be filtered by tenants in properties managed by the broker
    const myTenants = mockTenants.filter(t => t.status === 'Active');

    return (
        <AdminPageLayout title="My Tenants">
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {myTenants.map((tenant) => (
                            <tr key={tenant.id} className="border-b border-secondary-100 last:border-0 hover:bg-secondary-50 cursor-pointer" onClick={() => navigate(`/broker/tenants/${tenant.id}`)}>
                                <td className="p-3">
                                ...
                                <td className="p-3">
                                    <button onClick={(e) => {e.stopPropagation(); navigate(`/broker/tenants/${tenant.id}`)}} className="p-1.5 text-secondary-500 hover:text-blue-600"><Eye size={16} /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
