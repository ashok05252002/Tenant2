...
const LandlordApprovals = () => {
    const [applications, setApplications] = useState(mockPartnerApplications.filter(app => app.type === 'Landlord' && app.status === 'Pending'));
    ...
    return (
        <AdminPageLayout title="Landlord Approvals">
            <div className="bg-white rounded-xl shadow-sm border border-secondary-200 overflow-x-auto">
                ...
                {applications.length === 0 && <p className="p-4 text-center text-secondary-500">No pending landlord applications.</p>}
            </div>
        </AdminPageLayout>
    );
};
